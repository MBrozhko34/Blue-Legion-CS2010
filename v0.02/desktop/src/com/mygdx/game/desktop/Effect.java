...
public class Effect {
	//contains=[0]damage,[1]effect,[2]effect val,[3]Duration,[4]player it applies to
	private ArrayList<String[]> effectList;
	
	public Effect() {effectList=new ArrayList<String[]>();}
	
	/**
	 * This method iterates through the entire array deducting 1 from the 'duration' value. In the event that the value is 0,
	...
	private void update() {
		for(int c=0;c<=this.effectList.size()-1;c++) {
			...
			if(this.effectList.get(c)[3]=="0") {this.effectList.remove(c); c=c-1;}
			//if the above condition is not fulfilled then the duration of the effect's duration
			//is reduced by 1 turn
			else {this.effectList.get(c)[3]=Float.toString(Float.parseFloat(effectList.get(c)[3])-1);}
		}
	}
	/**
	 ...
	 */
	public ArrayList<String[]> getEffectList() {return this.effectList;}
	/**
	 * this method preforms a union operation between the local arrayList and the foreign arraylist
	 * 
	 * @param	Player	Takes the player2 as a parameter as a means of retrieving thier ActionList
	 */
